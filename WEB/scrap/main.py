...
def scrap_pexels(query=''):
    ...
    if not json_data.get('next_page'):
        ...
        logging.info('Всего изображений: {images_count}. Сохранение может занять какое-то время.'))
        ...
        print(images_list_urls, img_dir_path)
...
def download_images(img_list=[], img_dir_path=''):
    for item_url in tqdm(img_list):
        ...
        if response.status_code == 200:
            ...
            logging.info('Что-то пошло не так при скачивании изображения!')
